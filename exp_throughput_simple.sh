...
do
	...
	do
		for bpk in ${bpk_list[@]}
		do
			echo "./simple_benchmark -T ${T} -E ${E} --dd -p ${DB_HOME} --qwp ../workload_generator_scripts/Z${Z}_ZD${ZD}_simple_mixed_query_workload.txt -B ${B} -P ${P} -b ${bpk} --BCC ${BCC} -V 1 --no_dynamic_cmpct --dw --dr --clct-tputi ${throughput_interval} > ${dir}/Z${Z}_ZD${ZD}_bpk-${bpk}_output_simple_mixed_workload.txt"
			./simple_benchmark -T ${T} -E ${E} --dd -p ${DB_HOME} --qwp ../workload_generator_scripts/Z${Z}_ZD${ZD}_simple_mixed_query_workload.txt -B ${B} -P ${P} -b ${bpk} --BCC ${BCC} -R ${R} -V 1 --no_dynamic_cmpct --dw --dr --clct-tputi ${throughput_interval} > ${dir}/Z${Z}_ZD${ZD}_bpk-${bpk}_output_simple_mixed_workload.txt
