...
do
	if [ ${ZD} == "1" ]; then
		...
		do
			...
			./bpk_benchmark -p ${DB_HOME} -E${E} -P${P} -T${T} -B ${B} --no_dynamic_cmpct -V 1 --BCC 524288 -b ${bpk} --dd --iwp ../workload_generator_scripts/ingestion_workload.txt --qwp ../workload_generator_scripts/Normal_Dis_NDEV${NDEV}_empty_query_workload.txt --dqs --olcs --query-stats-path ${output_dir}/Normal_Dis_NDEV${NDEV}_query_stats.txt --run-stats-op ${output_dir}/Normal_Dis_NDEV${NDEV}_output.txt
		done
	else
		...
		./bpk_benchmark -p ${DB_HOME} -E${E} -P${P} -T${T} -B ${B} --no_dynamic_cmpct -V 1 --BCC 524288 -b ${bpk} --dd --iwp ../workload_generator_scripts/ingestion_workload.txt --qwp ../workload_generator_scripts/Uniform_Dis_empty_query_workload.txt --dqs --olcs --query-stats-path ${output_dir}/Uniform_Dis_query_stats.txt --run-stats-op ${output_dir}/Uniform_Dis_output.txt
	fi
